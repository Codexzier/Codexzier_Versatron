...
void AppWlanScannerResultList::drawWlanItem(int x, int y, int index, int wlanItemIndex) {

    WlanItem* item = (*_wlanItems)[wlanItemIndex];
    int posY = y + 30 * index;
...
void AppWlanScannerResultList::addWlanItem(WlanItem *item) {
    ...
    _wlanItems->push_back(item);

    _pageCount = static_cast<int>(_wlanItems->size()) / _itemsPerPage;
}
